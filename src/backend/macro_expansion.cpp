...
void processIRLine(const vector<string>::iterator irLineIter, vector<string>& asmLines, map<string, map<string, int>>& offsetTable, 
    ...
    if (tokens[0][0] == '@') {
        ...
        asmLines.push_back("\t\t.align 3");
        asmLines.push_back("\t\t.type " + var + ", @object");
        asmLines.push_back("\t\t.size " + var + ", " + to_string(8 * num));
        asmLines.push_back(var + ":");
        asmLines.push_back("\t\t.zero " + to_string(8 * num));
        ...
        if (tokens[3] == "alloca") {
            if (count < func_infos[now_func].arg.size()) {
                ...
                if (func_infos[now_func].arg_is_ptr[count]) {
                    asmLines.push_back("\t\tld %t, " + tokens[1]);
                    asmLines.push_back("\t\tli %tt, ~fs");
                    asmLines.push_back("\t\tadd %ttt, %t, %tt");
                    asmLines.push_back("\t\tsd %ttt, " + tokens[1]);
                }
                offsetTable[now_func][tokens[1]] = -8 * (count + 1);
                count++;
            } else {
                int alloc_num = stoi(tokens[5]);
                string alloc_type = tokens[4];
                sp_offset -= 8 * alloc_num;
                offsetTable[now_func][tokens[1]] = sp_offset;
            }
            ...
            if (src[0] == '#') {
                ...
                if (dst[0] == '@') {
                    ...
                    asmLines.push_back("\t\tsd " + src + ", 0(%t)");
                } else {
                    asmLines.push_back("\t\tsd " + src + ", " + dst); 
                }
            } else {
                if (dst[0] == '@') {
                    asmLines.push_back("\t\tla %t, " + dst);
                    asmLines.push_back("\t\tli %tt, " + src);
                    asmLines.push_back("\t\tsd %tt, 0(%t)");
                } else {
                    asmLines.push_back("\t\tli %t, " + src);
                    asmLines.push_back("\t\tsd %t, " + dst); 
                }
            }
            ...
            // sp_offset -= 4;
            // asmLines.push_back("\t\tsd " + dst + ", " + to_string(sp_offset) + "(sp)");
            ...
            for (int i = 5; i < (int)tokens.size(); i++) {
                if (tokens[i][0] != '%') {
                    if (tokens[i][0] == '@') {
                        ...
                        if (func_infos[func].arg_is_ptr[i-5]) {
                            ...
                            asmLines.push_back("\t\tsd %t, -" + to_string(8 * (i- 4)) + "(sp)");  
                        } else {
                            if (i-5 < 8) {
                                asmLines.push_back("\t\tmv a" + to_string(i-5) + ", " + tokens[i]);
                            }
                            asmLines.push_back("\t\tsd " + tokens[i] + ", -" + to_string(8 * (i- 4)) + "(sp)");
                        }
                ...
                // if it is an array
                    ...
                    // if it is from caller 
                        ...
                        asmLines.push_back("\t\tld %t, " + tokens[i]);
                        if (i-5 < 8) {
                            asmLines.push_back("\t\tmv a" + to_string(i-5) + ", %t");
                        }
                        asmLines.push_back("\t\tsd %t, -" + to_string(8 * (i- 4)) + "(sp)");
